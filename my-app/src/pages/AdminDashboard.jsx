...
export default function AdminDashboard() {
  ...
  /* ── Real-time listeners (only start once authenticated) ── */
  useEffect(() => {
    if (!authed) return;
    ...
    return (
      <div style={{ ...S.page, display:"flex", alignItems:"center", justifyContent:"center" }}>
        <div style={{ textAlign:"center", maxWidth:"320px" }}>
          <div style={{ fontSize:"14px", color:"#ff7070", fontWeight:"700", marginBottom:"8px" }}>
            Couldn't connect to the store
          </div>
          <div style={{ fontSize:"12px", color:"rgba(255,255,255,0.4)" }}>
            Sign-in failed. Check your Firebase config and internet connection, then reload.
          </div>
        </div>
      </div>
    );
